...
class PodSecurityContext extends AbstractModel
{
     ...
     * volumes used by the Pod. It has no effect on nodes that do not support SELinux
     * or to volumes does not support SELinux. Valid values are "MountOption" and
     * "Recursive".
     *
     ...
     * Pods. Eligible volumes are in-tree FibreChannel and iSCSI volumes, and all CSI
     * volumes whose CSI driver announces SELinux support by setting spec.seLinuxMount:
     * true in their CSIDriver instance. Other volumes are always re-labelled
     * recursively. "MountOption" value is allowed only when SELinuxMount feature gate
     * is enabled.
     *
     * If not specified and SELinuxMount feature gate is enabled, "MountOption" is
     * used. If not specified and SELinuxMount feature gate is disabled, "MountOption"
     ...
     * @var string
     */
    public $seLinuxChangePolicy = null;
     ...
     * to the container's primary GID and fsGroup (if specified).  If the
     ...
     * defined in the container image. If unspecified, no additional groups are added,
     * though group memberships defined in the container image may still be used,
     * depending on the supplementalGroupsPolicy field. Note that this field cannot be
     ...
     * (Alpha) Using the field requires the SupplementalGroupsPolicy feature gate to be
     * enabled and the container runtime must implement support for this feature. Note
