<?php

namespace Kubernetes\API;

use \KubernetesRuntime\AbstractAPI;
use \Kubernetes\Model\Io\K8s\Api\Storagemigration\V1alpha1\StorageVersionMigrationList as StorageVersionMigrationList;
use \Kubernetes\Model\Io\K8s\Api\Storagemigration\V1alpha1\StorageVersionMigration as TheStorageVersionMigration;
use \Kubernetes\Model\Io\K8s\Apimachinery\Pkg\Apis\Meta\V1\Status as Status;
use \Kubernetes\Model\Io\K8s\Apimachinery\Pkg\Apis\Meta\V1\WatchEvent as WatchEvent;

class StorageVersionMigration extends AbstractAPI
{
    /**
     * list or watch objects of kind StorageVersionMigration
     *
     * @return StorageVersionMigrationList|mixed
     */
    public function list()
    {
        return $this->parseResponse(
        	$this->client->request('get',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations",
        		[
        		]
        	),
        	'listStoragemigrationV1alpha1StorageVersionMigration'
        );
    }

    /**
     * create a StorageVersionMigration
     *
     * @param TheStorageVersionMigration $Model
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     * 'fieldValidation'	string
     * fieldValidation instructs the server on how to handle objects in the request
     * (POST/PUT/PATCH) containing unknown or duplicate fields. Valid values are: -
     * Ignore: This will ignore any unknown fields that are silently dropped from the
     * object, and will ignore all but the last duplicate field that the decoder
     * encounters. This is the default behavior prior to v1.23. - Warn: This will send
     * a warning via the standard warning response header for each unknown field that
     * is dropped from the object, and for each duplicate field that is encountered.
     * The request will still succeed if there are no other errors, and will only
     * persist the last of any duplicate fields. This is the default in v1.23+ -
     * Strict: This will fail the request with a BadRequest error if any unknown fields
     * would be dropped from the object, or if any duplicate fields are present. The
     * error returned from the server will contain all unknown and duplicate fields
     * encountered.
     *
     * @return TheStorageVersionMigration|mixed
     */
    public function create(\Kubernetes\Model\Io\K8s\Api\Storagemigration\V1alpha1\StorageVersionMigration $Model, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('post',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations",
        		[
        			'json' => $Model->getArrayCopy(),
        			'query' => $queries,
        		]
        	),
        	'createStoragemigrationV1alpha1StorageVersionMigration'
        );
    }

    /**
     * delete collection of StorageVersionMigration
     *
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     *
     * @return Status|mixed
     */
    public function deleteCollection(array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('delete',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations",
        		[
        			'query' => $queries,
        		]
        	),
        	'deleteStoragemigrationV1alpha1CollectionStorageVersionMigration'
        );
    }

    /**
     * read the specified StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @return TheStorageVersionMigration|mixed
     */
    public function read(string $name)
    {
        return $this->parseResponse(
        	$this->client->request('get',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}",
        		[
        		]
        	),
        	'readStoragemigrationV1alpha1StorageVersionMigration'
        );
    }

    /**
     * replace the specified StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @param TheStorageVersionMigration $Model
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     * 'fieldValidation'	string
     * fieldValidation instructs the server on how to handle objects in the request
     * (POST/PUT/PATCH) containing unknown or duplicate fields. Valid values are: -
     * Ignore: This will ignore any unknown fields that are silently dropped from the
     * object, and will ignore all but the last duplicate field that the decoder
     * encounters. This is the default behavior prior to v1.23. - Warn: This will send
     * a warning via the standard warning response header for each unknown field that
     * is dropped from the object, and for each duplicate field that is encountered.
     * The request will still succeed if there are no other errors, and will only
     * persist the last of any duplicate fields. This is the default in v1.23+ -
     * Strict: This will fail the request with a BadRequest error if any unknown fields
     * would be dropped from the object, or if any duplicate fields are present. The
     * error returned from the server will contain all unknown and duplicate fields
     * encountered.
     *
     * @return TheStorageVersionMigration|mixed
     */
    public function replace(string $name, \Kubernetes\Model\Io\K8s\Api\Storagemigration\V1alpha1\StorageVersionMigration $Model, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('put',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}",
        		[
        			'json' => $Model->getArrayCopy(),
        			'query' => $queries,
        		]
        	),
        	'replaceStoragemigrationV1alpha1StorageVersionMigration'
        );
    }

    /**
     * delete a StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     *
     * @return Status|mixed
     */
    public function delete(string $name, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('delete',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}",
        		[
        			'query' => $queries,
        		]
        	),
        	'deleteStoragemigrationV1alpha1StorageVersionMigration'
        );
    }

    /**
     * partially update the specified StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     * 'fieldValidation'	string
     * fieldValidation instructs the server on how to handle objects in the request
     * (POST/PUT/PATCH) containing unknown or duplicate fields. Valid values are: -
     * Ignore: This will ignore any unknown fields that are silently dropped from the
     * object, and will ignore all but the last duplicate field that the decoder
     * encounters. This is the default behavior prior to v1.23. - Warn: This will send
     * a warning via the standard warning response header for each unknown field that
     * is dropped from the object, and for each duplicate field that is encountered.
     * The request will still succeed if there are no other errors, and will only
     * persist the last of any duplicate fields. This is the default in v1.23+ -
     * Strict: This will fail the request with a BadRequest error if any unknown fields
     * would be dropped from the object, or if any duplicate fields are present. The
     * error returned from the server will contain all unknown and duplicate fields
     * encountered.
     *
     * @return TheStorageVersionMigration|mixed
     */
    public function patch(string $name, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('patch',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}",
        		[
        			'query' => $queries,
        		]
        	),
        	'patchStoragemigrationV1alpha1StorageVersionMigration'
        );
    }

    /**
     * read status of the specified StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @return TheStorageVersionMigration|mixed
     */
    public function readStatus(string $name)
    {
        return $this->parseResponse(
        	$this->client->request('get',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}/status",
        		[
        		]
        	),
        	'readStoragemigrationV1alpha1StorageVersionMigrationStatus'
        );
    }

    /**
     * replace status of the specified StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @param TheStorageVersionMigration $Model
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     * 'fieldValidation'	string
     * fieldValidation instructs the server on how to handle objects in the request
     * (POST/PUT/PATCH) containing unknown or duplicate fields. Valid values are: -
     * Ignore: This will ignore any unknown fields that are silently dropped from the
     * object, and will ignore all but the last duplicate field that the decoder
     * encounters. This is the default behavior prior to v1.23. - Warn: This will send
     * a warning via the standard warning response header for each unknown field that
     * is dropped from the object, and for each duplicate field that is encountered.
     * The request will still succeed if there are no other errors, and will only
     * persist the last of any duplicate fields. This is the default in v1.23+ -
     * Strict: This will fail the request with a BadRequest error if any unknown fields
     * would be dropped from the object, or if any duplicate fields are present. The
     * error returned from the server will contain all unknown and duplicate fields
     * encountered.
     *
     * @return TheStorageVersionMigration|mixed
     */
    public function replaceStatus(string $name, \Kubernetes\Model\Io\K8s\Api\Storagemigration\V1alpha1\StorageVersionMigration $Model, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('put',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}/status",
        		[
        			'json' => $Model->getArrayCopy(),
        			'query' => $queries,
        		]
        	),
        	'replaceStoragemigrationV1alpha1StorageVersionMigrationStatus'
        );
    }

    /**
     * partially update status of the specified StorageVersionMigration
     *
     * @param string $name name of the StorageVersionMigration
     * @param array $queries options:
     * 'dryRun'	string
     * When present, indicates that modifications should not be persisted. An invalid
     * or unrecognized dryRun directive will result in an error response and no further
     * processing of the request. Valid values are: - All: all dry run stages will be
     * processed
     * 'fieldValidation'	string
     * fieldValidation instructs the server on how to handle objects in the request
     * (POST/PUT/PATCH) containing unknown or duplicate fields. Valid values are: -
     * Ignore: This will ignore any unknown fields that are silently dropped from the
     * object, and will ignore all but the last duplicate field that the decoder
     * encounters. This is the default behavior prior to v1.23. - Warn: This will send
     * a warning via the standard warning response header for each unknown field that
     * is dropped from the object, and for each duplicate field that is encountered.
     * The request will still succeed if there are no other errors, and will only
     * persist the last of any duplicate fields. This is the default in v1.23+ -
     * Strict: This will fail the request with a BadRequest error if any unknown fields
     * would be dropped from the object, or if any duplicate fields are present. The
     * error returned from the server will contain all unknown and duplicate fields
     * encountered.
     *
     * @return TheStorageVersionMigration|mixed
     */
    public function patchStatus(string $name, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('patch',
        		"/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/{$name}/status",
        		[
        			'query' => $queries,
        		]
        	),
        	'patchStoragemigrationV1alpha1StorageVersionMigrationStatus'
        );
    }

    /**
     * watch individual changes to a list of StorageVersionMigration. deprecated: use
     * the 'watch' parameter with a list operation instead.
     *
     * @return WatchEvent|mixed
     */
    public function watchList()
    {
        return $this->parseResponse(
        	$this->client->request('get',
        		"/apis/storagemigration.k8s.io/v1alpha1/watch/storageversionmigrations",
        		[
        		]
        	),
        	'watchStoragemigrationV1alpha1StorageVersionMigrationList'
        );
    }

    /**
     * watch changes to an object of kind StorageVersionMigration. deprecated: use the
     * 'watch' parameter with a list operation instead, filtered to a single item with
     * the 'fieldSelector' parameter.
     *
     * @param string $name name of the StorageVersionMigration
     * @return WatchEvent|mixed
     */
    public function watch(string $name)
    {
        return $this->parseResponse(
        	$this->client->request('get',
        		"/apis/storagemigration.k8s.io/v1alpha1/watch/storageversionmigrations/{$name}",
        		[
        		]
        	),
        	'watchStoragemigrationV1alpha1StorageVersionMigration'
        );
    }
}

